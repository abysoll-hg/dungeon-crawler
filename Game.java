...
public class Game {   
   public static void main(String[] args) {
      ...
      while (exitGame != true) {
         ...
         // do this.
            ...
            for (Item i : player.getRoom(dungeonMap).getItems()) {
               if (item.equalsIgnoreCase(i.toString())) {
                  ...
                  System.out.println("You took the " + i.toString() + ".");
                  itemFound = true;
                  break;
               }
            }
         ...
         if (itemToDrop != null) {
            inventory.removeItemFromInventory(itemToDrop);
            player.getRoom(dungeonMap).addItem(itemToDrop);
            System.out.println("Dropped "+itemToDrop.toString()+".");
         } 
         else {
         ...
         /*else if (input.startsWith("FIGHT ")) {
            String monster = input.substring(5);
            boolean existsInRoom = false;
            for (Monster m : player.getRoom(dungeonMap).getMonsters()) {
            }
         }*/
      }
   }
